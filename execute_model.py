...
def make_colors(n):
    # genera n colores distintos (simple rampa HSV→BGR)
    cols = []
    for i in range(n):
        hue = int(180 * i / max(1, n))  # [0,180) en OpenCV
        col = cv2.cvtColor(np.uint8([[[hue, 200, 255]]]), cv2.COLOR_HSV2BGR)[0,0]
        cols.append((int(col[0]), int(col[1]), int(col[2])))
    return cols

colors = make_colors(len(words))

def prob_viz(res, actions, input_frame, colors, scale=300):
    output_frame = input_frame.copy()
    for num, prob in enumerate(res):
        y1 = 60 + num*25
        y2 = y1 + 20
        x2 = int(float(prob) * scale)
        cv2.rectangle(output_frame, (0, y1), (x2, y2), colors[num], -1)
        cv2.putText(output_frame, f"{actions[num]} {prob:.2f}", (5, y2-5),
